    ...
    <div class="card bg-secondary shadow h-100">
        <div class="card-body">
            <form autocomplete="off" action="{{ route('jurnal-umum.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <div class="col-sm-4">
                        <button type="button" class="addDebit btn btn-primary" id="add-btn"> <i class="fas fa-plus"></i>&nbsp; Add More Data</button>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    <label class="form-control-label col-md-3" for="debit_kredit">Debit/Kredit</label>
                    <div class="col-md-3">
                        <select class="form-control" name="debit_atau_kredit[]" id="debit_kredit">
                ...
                <div class="debit"></div>
                <button type="submit" class="btn btn-info btn-block" id="simpan">SIMPAN</button>
            </form>
        </div>
    </div>
...
<script>
    ...
    function addData(){
        let debit = '<div><div class="form-group row"><label class="form-control-label col-md-3" for="akun_id">Akun</label><div class="col-md-9"><select class="form-control" name="akun_id[]" id="akun_id">@foreach (App\Models\Akun::orderBy("kode")->get() as $item)<option value="{{ $item->id }}" {{ old("akun_id") == $item->id ? "selected" : '' }}>{{ $item->kode }} - {{ $item->nama }}</option>@endforeach</select><span class="invalid-feedback font-weight-bold"></span></div></div> <div class="form-group row"><label class="form-control-label col-md-3" for="debit_kredit">Debit/Kredit</label><div class="col-md-3"><select class="form-control" name="debit_atau_kredit[]" id="debit_kredit"><option selected disabled value="">-- Select ---</option><option value="1">Debit</option><option value="2">Kredit</option></select><span class="invalid-feedback font-weight-bold"></span></div></div><div class="row mb-2"><div class="col-sm-4"><button type="button" class="rmData btn btn-danger"><i class="fas fa-plus"></i>&nbsp; Delete Data</button></div></div></div>';
        $('.debit').append(debit);
    };
    $(document).on('click', '.rmData', function(){
