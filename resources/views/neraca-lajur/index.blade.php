                ...
                <div class="tab-pane fade" id="pills-penyesuaian" role="tabpanel" aria-labelledby="pills-penyesuaian-tab">
                    <div class="table-responsive">
                        <table class="table table-hover table-sm table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse ($akun as $items)
                                    @php
                                        $data = neraca(request('kriteria'), request('periode'), request('tanggal_awal'), request('tanggal_akhir'), request('bulan'), $items);
                                    @endphp
                                    <tr>
                                        <td style="vertical-align: middle; text-align: center"><a href="{{ url('/buku-besar?kode_akun=' . $items->kode . '&kriteria=periode&periode=1-bulan-terakhir') }}">{{ $items->kode }}</a></td>
                                        <td>{{ $items->nama }}</td>
                                        <td class="text-right penyesuaian_debit">{{ $items->post_penyesuaian == 1 ? 'Rp. ' . substr(number_format($data['penyesuaian'], 2, ',', '.'),0,-3) : '-' }}</td>
                                        <td class="text-right penyesuaian_kredit">{{ $items->post_penyesuaian == 2 ? 'Rp. ' . substr(number_format($data['penyesuaian'], 2, ',', '.'),0,-3) : '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="15" align="center">Data tidak tersedia</td>
                        ...
                        <table class="table table-hover table-sm table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse ($akun as $item)
                                    ...
                                    <tr>
                                        @if ($kode != 1110 && $kode != 1120 && $kode != 1130 && $kode != 1140 && $kode != 1210 && $kode != 1211 && $kode != 1220 && $kode != 1221 && $kode != 1230 && $kode != 1231 && $kode != 1240 && $kode != 2110 && $kode != 2120 && $kode != 3110 && $kode != 3120)
                                        <td style="vertical-align: middle; text-align: center"><a href="{{ url('/buku-besar?kode_akun=' . $item->kode . '&kriteria=periode&periode=1-bulan-terakhir') }}">{{ $item->kode }}</a></td>
                                        <td>{{ $item->nama }}</td>
                            ...
                            <tfoot class="bg-primary text-white">
                                ...
                                <tr>
                                    <th class="text-right" colspan="2">Selisih</th>
                                    <th class="text-right" id="selisih_laba_rugi"></th>
                                    <th></th>
                                </tr>
                                <tr>
                                    <th colspan="2"></th>
                                    <th class="text-right" id="total_laba_rugi_debit"></th>
                                    <th class="text-right" id="total_laba_rugi_kredit"></th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                        ...
                        <table class="table table-hover table-sm table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse ($akun as $item)
                                    ...
                                    @endphp
                                    <tr>
                                        @if ($kode != 5130 && $kode != 6110 && $kode != 6120 && $kode != 6140 && $kode != 6150 && $kode != 6160 && $kode != 6170 && $kode != 6180 && $kode != 6130 && $kode != 4110 && $kode != 5110 && $kode != 5111 && $kode != 5120 && $kode != 6181 && $kode != 6182 && $kode != 6190)
                                        <td style="vertical-align: middle; text-align: center"><a href="{{ url('/buku-besar?kode_akun=' . $item->kode . '&kriteria=periode&periode=1-bulan-terakhir') }}">{{ $item->kode }}</a></td>
                                        <td>{{ $item->nama }}</td>
                                            @if ($item->post_laporan == 2)
                            ...
                            <tfoot class="bg-primary text-white">
                                ...
                                <tr>
                                    ...
                                    <th></th>
                                    <th class="text-right" id="selisih_neraca"></th>
                                </tr>
                                <tr>
                                    <th colspan="2" class="text-right"></th>
                                    <th class="text-right" id="total_neraca_debit"></th>
                                    <th class="text-right" id="total_neraca_kredit"></th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        $("#total_laba_rugi_debit").html('Rp. ' + new Intl.NumberFormat('id-ID').format(angka($("#jumlah_laba_rugi_debit").html()) + angka($("#selisih_laba_rugi").html())));
        $("#total_laba_rugi_kredit").html('Rp. ' + new Intl.NumberFormat('id-ID').format(jumlah('laba_rugi_kredit')));
        ...
        $("#total_neraca_debit").html('Rp. ' + new Intl.NumberFormat('id-ID').format(jumlah('neraca_debit')));
        $("#total_neraca_kredit").html('Rp. ' + new Intl.NumberFormat('id-ID').format(Math.abs(angka($("#jumlah_neraca_kredit").html()) + angka($("#selisih_neraca").html()))));
        kriteria();
